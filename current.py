...
class Solution:
    def leastInterval(self, tasks: List[str], n: int) -> int:
        counts = list([(0, count, letter) for letter, count in Counter(tasks).items()])
        heapify(counts)
        draw_heap(counts)
        res = []
        i = 0
        while counts:
            i += 1
            cooloff, count, task = heappop(counts)
            if cooloff < i:
                res.append(task)
                count -= 1
                if count > 0:
                    heappush(counts, (i + n, count, task))
            else:
                heappush(counts, (cooloff, count, task))
                res.append("idle")
            print(res)
            draw_heap(counts)
        print(res)
