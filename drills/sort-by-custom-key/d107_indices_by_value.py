...
ordered so that their nums2 values run from largest to smallest. When two
...
Input: nums2 = [2, 1, 3, 4]
Output: [3, 2, 0, 1]
Explanation: nums2[3] = 4, nums2[2] = 3, nums2[0] = 2, nums2[1] = 1.
...
print(sol.indicesByValue([2, 1, 3, 4]))  # [3, 2, 0, 1]

# assert sol.indicesByValue([2, 1, 3, 4]) == [3, 2, 0, 1]
# assert sol.indicesByValue([7, 5, 10, 9, 6]) == [2, 3, 0, 4, 1]
# assert sol.indicesByValue([3, 1, 3]) == [0, 2, 1]
# assert sol.indicesByValue([5, 5, 5]) == [0, 1, 2]
# assert sol.indicesByValue([0]) == [0]
# assert sol.indicesByValue(list(range(100000))) == list(range(99999, -1, -1))
