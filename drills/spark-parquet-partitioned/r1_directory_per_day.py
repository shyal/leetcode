"""
DRILL: Directory Per Day
TRAINS: spark-parquet-partitioned

Given the DataFrame `solves` and an empty directory `path`, write `solves`
there as Parquet laid out one directory per `date`. Return nothing.

DataFrame: solves

    +-------------+---------+
    | Column Name | Type    |
    +-------------+---------+
    | file        | string  |
    | date        | string  |
    | problem     | string  |
    +-------------+---------+
    file is unique.

Example 1:

Input:
solves:
+------+------------+---------+
| file | date       | problem |
+------+------------+---------+
| a.py | 2026-08-28 | 1       |
| b.py | 2026-08-28 | 42      |
| c.py | 2026-08-29 | 76      |
+------+------------+---------+
Output: under path,
    date=2026-08-28/....parquet
    date=2026-08-29/....parquet
and reading path back yields the three rows.

Example 2:

Input:
solves:
+------+------------+---------+
| file | date       | problem |
+------+------------+---------+
| a.py | 2026-08-29 | 1       |
+------+------------+---------+
Output: under path, date=2026-08-29/....parquet.

Constraints:

    1 <= number of rows <= 10^4
    1 <= number of distinct dates <= 100

    REQUIRED: one directory per distinct date, named date=<value>, with
    every data file inside one of them; an unpartitioned write puts the
    files directly under path and fails. NO collect(), NO Python file
    writing.

    Runner: local PySpark, adaptive execution and auto-broadcast off.
    dsa.spark.tree(path) lists the files written, relative to path.
"""

from pyspark.sql import functions as F

from dsa.spark import SparkDrill, scratch_dir, tree


class Solution(SparkDrill):

    engine = "jvm"  # the file layout on disk is Spark's own

    def write(self, solves, path: str) -> None:
        pass


SOLVES = "file string, date string, problem string"

EXAMPLE_1 = {
    "solves": (
        [
            ("a.py", "2026-08-28", "1"),
            ("b.py", "2026-08-28", "42"),
            ("c.py", "2026-08-29", "76"),
        ],
        SOLVES,
    ),
}

EXAMPLE_2 = {
    "solves": (
        [
            ("a.py", "2026-08-29", "1"),
        ],
        SOLVES,
    ),
}


def layout(example):
    path = scratch_dir()
    sol.write(sol.frames(example)["solves"], path)
    files = tree(path)
    if not files:
        return [], []
    rows = sorted(tuple(r) for r in sol.spark.read.parquet(path).collect())
    return files, rows


sol = Solution()

files, rows = layout(EXAMPLE_1)
print("\n".join(files) or "(nothing written)")

# files, rows = layout(EXAMPLE_1)
# assert sorted({f.split("/")[0] for f in files}) == ["date=2026-08-28", "date=2026-08-29"]
# assert all(f.endswith(".parquet") for f in files)
# assert rows == [("a.py", "1", "2026-08-28"), ("b.py", "42", "2026-08-28"), ("c.py", "76", "2026-08-29")]
# files, rows = layout(EXAMPLE_2)
# assert sorted({f.split("/")[0] for f in files}) == ["date=2026-08-29"]
