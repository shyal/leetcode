...
Output:
+-----------------+------------+-----------+------+
| node_id         | date       | verdict   | gap  |
+-----------------+------------+-----------+------+
| monotonic-stack | 2026-08-01 | struggled | NULL |
| monotonic-stack | 2026-08-10 | clean     | 9    |
| monotonic-stack | 2026-08-20 | clean     | 10   |
...
+--------------+------------+---------+------+
| two-pointers | 2026-08-05 | clean   | NULL |
| two-pointers | 2026-08-06 | clean   | 1    |
+--------------+------------+---------+------+
...
Constraints:
    ...
    REQUIRED: every row survives with the gap to its own node's previous
    row; the first row of a node must be null, not 0. NO self-join, NO
    collect().
...
# assert sol.run(EXAMPLE_1) == [("monotonic-stack", date(2026, 8, 1), "struggled", None), ("monotonic-stack", date(2026, 8, 10), "clean", 9), ("monotonic-stack", date(2026, 8, 20), "clean", 10), ("two-pointers", date(2026, 8, 5), "clean", None)]
# assert sol.run(EXAMPLE_2) == [("two-pointers", date(2026, 8, 5), "clean", None), ("two-pointers", date(2026, 8, 6), "clean", 1)]
