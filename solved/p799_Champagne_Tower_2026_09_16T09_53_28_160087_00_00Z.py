...
This one killed me first time. Turns out to be a simulation, which might make it a
touch more approachable.

Right.. i remember the key insight. The top glass holds all the liquid, then
at each level, we simulate it flowing down..

       U
      U U

The query is the row and the glass, so the outer loop can be the row,
decreasing until we hit 0, and the inner loop grows by 1, so is the
same size as the row, but we stop once the row has hit 0 and the glass
has hit the column.
...
LEETCODE: Accepted (31 ms, 19.6 MB)
"""
...
print(sol.champagneTower(1, 5, 1))  # 0.0
