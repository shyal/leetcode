...
This is what sol.union_shared(accounts) does, and it's been run for you.
...
Then, to find the enigmatic `person`: call find on the account. This is
because we ran sol.union_shared(accounts) earlier, so the person for that
account may be different.

What we're really interested in knowing is, which person owns which email.

So add each email address to its rightful owner in the persons[person]
set.
