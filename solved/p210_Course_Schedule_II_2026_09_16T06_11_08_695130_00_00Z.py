...
Ok this can be solved using a BFS, i think.

- build an adjacency graph
- compute in-degrees for all courses
- courses with an in-degree of 0 are the starting points for the bfs
- no need for multi source bfs, just keep a visited

Ok i got the adj graph right, i got the in_degrees list right, i populated
the queue right, but wasn't sure what to do at the BFS stage.

So we pop left, explore neighbours which i get, but we decrement the in_degrees
and only append to the queue if the in degree is 0, which feels a bit mystical
but i'm sure will click soon.

Learning.

LEETCODE: Accepted (4 ms, 20.3 MB)
"""
