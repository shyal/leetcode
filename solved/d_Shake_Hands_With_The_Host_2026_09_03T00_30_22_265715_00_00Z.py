...
first to arrive hosts the party. Every other guest shakes hands with the
host. The dict adj is one record over all parties: every guest is a key,
and adj[g] holds every guest g shook hands with, at any party, in the
order the handshakes happen, repeats included.
    ...
    def handshakes(self, parties: List[List[int]]) -> Dict[int, List[int]]:
        ...
        for guests in parties:
            ...
            for guest in guests[1:]:
                adj[host].append(guest)
                adj[guest].append(host)
        return adj
