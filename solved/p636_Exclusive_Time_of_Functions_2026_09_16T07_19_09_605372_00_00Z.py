...
Most asserts pass, though not the ones where a function runs multiple times in a row:
    ...
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            ...
            if op == "start":
                ...
                started_id, started_ts = stack.pop()
                delta = ts - started_ts + 1
                times[started_id] += delta
                stack[-1][1] += delta
        return times

Had to be hinted about tracking the child. Rest of the solve was mine.

LEETCODE: Accepted (6 ms, 19.4 MB)
"""
    ...
    def exclusiveTime(self, n: int, logs: List[str]) -> List[int]:
        ...
        for log in logs:
            ...
            if op == "start":
                ...
                started_id, started_ts, child = stack.pop()
                span = ts - started_ts + 1
                times[started_id] += span - child
                stack[-1][2] += span
        return times
