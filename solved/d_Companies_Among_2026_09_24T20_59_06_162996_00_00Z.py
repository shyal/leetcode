...
#   len({self.find y for y in employees})

class Solution(UnionFind):
    def countCompaniesAmong(self, employees: list[int]) -> int:
        return len({self.find(y) for y in employees})


sol = Solution(6)
sol.parent = [1, 2, 2, 4, 5, 5]
print(sol.countCompaniesAmong([0, 3, 1]))
sol = Solution(6)
sol.parent = [1, 2, 2, 4, 5, 5]
...
sol = Solution(1000)
assert sol.countCompaniesAmong(list(range(0, 1000))) == 1000
