"""
URL: https://leetcode.com/problems/sum-of-all-odd-length-subarrays/description/

1588. Sum of All Odd Length Subarrays

Given an array of positive integers arr, return the sum of all possible odd-length subarrays.

A subarray is a contiguous subsequence of the array.


Example 1:

Input: arr = [1,4,2,5,3]
Output: 58
Explanation: The odd-length subarrays of arr and their sums are:
[1] = 1
[4] = 4
[2] = 2
[5] = 5
[3] = 3
[1,4,2] = 7
[4,2,5] = 11
[2,5,3] = 10
[1,4,2,5,3] = 15
If we add all these up we get 1 + 4 + 2 + 5 + 3 + 7 + 11 + 10 + 15 = 58

Example 2:

Input: arr = [1,2]
Output: 3
Explanation: There are only 2 subarrays of odd length, [1] and [2]. Their sum is 3.

Example 3:

Input: arr = [10,11,12]
Output: 66


Constraints:

    1 <= arr.length <= 100
    1 <= arr[i] <= 1000

---

Had a think a little bit more for this problem. This definitely
feels like a brute force solution.

"""


class Solution:
    def sumOddLengthSubarrays(self, arr: List[int]) -> int:
        count = 0
        for i in range(1, len(arr) + 1, 2):
            for j in range(len(arr)):
                if i + j > len(arr):
                    break
                sub = arr[j : j + i]
                count += sum(sub)
        return count


sol = Solution()

# print(sol.sumOddLengthSubarrays([1, 4, 2, 5, 3]))  # 58

assert sol.sumOddLengthSubarrays([1, 4, 2, 5, 3]) == 58
assert sol.sumOddLengthSubarrays([1, 2]) == 3
assert sol.sumOddLengthSubarrays([10, 11, 12]) == 66
assert sol.sumOddLengthSubarrays([1]) == 1
assert sol.sumOddLengthSubarrays([1000]) == 1000
assert sol.sumOddLengthSubarrays([1, 1]) == 2
assert sol.sumOddLengthSubarrays([1, 1, 1]) == 6
assert sol.sumOddLengthSubarrays([1, 1, 1, 1]) == 10
assert sol.sumOddLengthSubarrays([1000, 1, 1000]) == 4002
