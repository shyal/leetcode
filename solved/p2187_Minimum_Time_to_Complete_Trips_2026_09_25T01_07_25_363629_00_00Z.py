...
LEETCODE: Accepted (1819 ms, 32.1 MB)
"""
...
#
# def minimumTime(time: [int], totalTrips: int) -> int
#   first_true(1, 10**15, x -> sum(x // y for y in time) >= totalTrips)
#

class Solution:
    def minimumTime(self, time: list[int], totalTrips: int) -> int:
        return first_true(1, 10 ** 15, lambda x: sum(x // y for y in time) >= totalTrips)
