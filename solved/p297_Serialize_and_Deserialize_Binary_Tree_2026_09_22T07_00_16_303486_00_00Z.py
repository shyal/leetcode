...
LEETCODE: Runtime Error (Line 25: NameError: name 'Codec' is not defined)
"""
...
class Solution:
    ...
    def serialize(self, root: Optional[TreeNode]) -> str:
        def dfs(n):
            res.append(str(n.val) if n else "null")
            if n:
                dfs(n.left)
                dfs(n.right)
    ...
    def deserialize(self, data: str) -> Optional[TreeNode]:
        def helper(it):
            ...
            if next_val in [None, "null"]:
                return
            return TreeNode(int(next_val), helper(it), helper(it))
