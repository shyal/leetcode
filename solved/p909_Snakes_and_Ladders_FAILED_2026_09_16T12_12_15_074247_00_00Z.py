...
class Solution:
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        def square_to_ij(x):
            div, mod = divmod(x - 1, len(board))
            if div % 2 == 0:
                return len(board) - div - 1, mod
            else:
                return len(board) - div - 1, len(board[0]) - 1 - mod

        print(square_to_ij(12))

        curr = 1
        n = len(board) * len(board[0])
        skip = False
        res = 0
        # next = curr + choice([curr + 1, min(curr + 6, n * n)])
        while True:
            print(curr)
            if curr >= 36:
                break
            res += 1
            i, j = square_to_ij(next)
            if board[i][j] != 1 and not skip:
                next = square_to_ij(board[i][j])
                curr = next
                skip = True
                continue
            skip = False
...
# FAILED: walked away after 25m 8s; no working solution.
