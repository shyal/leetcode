    ...
    def countKConstraintSubstrings(self, s: str, k: int) -> int:
        ...
        for i in range(len(s)):
            for j in range(i, len(s)):
                ss = s[i:j+1]
                num_0 = sum([x == '0' for x in ss])
                num_1 = len(ss) - num_0
                count += num_0 <= k or num_1 <= k
        return count
...
assert sol.countKConstraintSubstrings("01", 1) == 3
assert sol.countKConstraintSubstrings("10", 1) == 3
assert sol.countKConstraintSubstrings("0000", 1) == 10
assert sol.countKConstraintSubstrings("0011", 1) == 9
assert sol.countKConstraintSubstrings("0011", 4) == 10
assert sol.countKConstraintSubstrings("000111", 1) == 17
assert sol.countKConstraintSubstrings("110000", 1) == 18
assert sol.countKConstraintSubstrings("0001", 1) == 10
assert sol.countKConstraintSubstrings("010", 1) == 6
assert sol.countKConstraintSubstrings("0101", 2) == 10
assert sol.countKConstraintSubstrings("10101", 5) == 15
assert sol.countKConstraintSubstrings("1" * 50, 1) == 1275