...
#   [x[1] for x in sort((v, i) for i, v in nums2)]

class Solution:
    def indicesByValue(self, nums2: list[int]) -> list[int]:
        return [x[1] for x in sorted((v, i) for i, v in enumerate(nums2))]


sol = Solution()
print(sol.indicesByValue([2, 1, 3, 4]))
...
assert sol.indicesByValue(list(range(0, 100000))) == list(range(0, 100000))
