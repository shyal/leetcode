...
assert sol.nearestExit([["."]], [0, 0]) == -1
# assert sol.nearestExit([["." for _ in range(100)] for _ in range(100)], [50, 50]) == 49
assert sol.nearestExit([["+"] * 100 for _ in range(100)], [0, 0]) == -1
assert sol.nearestExit([["."] * 5 for _ in range(1)], [0, 2]) == 1
assert sol.nearestExit([["."] * 1 for _ in range(5)], [2, 0]) == 1
assert (
    sol.nearestExit(
        [["+", ".", "+", "."], [".", ".", ".", "+"], ["+", "+", "+", "."]], [1, 1]
    )
...
assert sol.nearestExit([["+", ".", "+"], ["+", ".", "+"], ["+", ".", "+"]], [1, 1]) == 1
assert sol.nearestExit([["."] * 10 for _ in range(10)], [0, 0]) == 1
assert sol.nearestExit([["."] * 10 for _ in range(10)], [9, 9]) == 1
assert (
    sol.nearestExit(
        [
            ["+", ".", "+", "+", "+"],
...
assert sol.nearestExit([["."] * 100], [0, 0]) == 1
