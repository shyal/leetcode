...
Worth noting this is really just a normal iteration over
a linked list, with a dummy curr instead of a head.

Instead of simply iterating, we check if next.val is the
val we want to avoid.

If it is, we skip it by setting curr.next to curr.next.next.

This actually doesn't iterate, it makes the unwanted node
unreachable and sets curr's next node to the next node in
the chain.

Else iteration is as usual.
