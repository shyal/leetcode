...
LEETCODE: Accepted (46 ms, 22.7 MB)
"""
...
class Solution:
    def hasCycle(self, head: Optional[ListNode]) -> bool:
        slow, fast = head, head
        count = 0
        while count <= 10**4 and fast and fast.next and fast.next.next:
            slow = slow.next
            fast = fast.next.next
            if slow == fast:
                return True
...
# [1,1,1,1], pos=-1: equal values, no cycle
head4 = build_linked_list([1, 1, 1, 1])
assert sol.hasCycle(head4) is False
