...
LEETCODE: Accepted (354 ms, 71.1 MB)
"""
...
class StockPrice:
    ...
    def update(self, timestamp: int, price: int) -> None:
        if timestamp in self.seen:
            self.updates[timestamp] = price
        maxheappush(self.vals, (timestamp, price))
        maxheappush(self.max, (price, timestamp))
        heappush(self.min, (price, timestamp))
        self.seen.add(timestamp)

    def current(self) -> int:
        while True:
            ts, val = maxheappeek(self.vals)
            if ts in self.updates and self.updates[ts] != val:
                maxheappop(self.vals)
                continue
            return int(val)

    def maximum(self) -> int:
        while True:
            val, ts = maxheappeek(self.max)
            if ts in self.updates and self.updates[ts] != val:
                maxheappop(self.max)
                continue
            return int(val)
