...
# mu 0.4
# def largestEven(nums: [int]) -> int
#   # python
#   max((x for x in nums if x % 2 == 0), default=-1)
#   # mu
#   max from -1 for x in nums if x % 2 == 0: x

class Solution:
    def largestEven(self, nums: list[int]) -> int:
        max((x for x in nums if x % 2 == 0), default=-1)
        return max(-1, max((x for x in nums if x % 2 == 0), default=-1))


sol = Solution()
print(sol.largestEven([3, 8, 5, 6, 1]))
