...
class StockPrice:
    ...
    def update(self, timestamp: int, price: int) -> None:
        is_update = timestamp in self.keys
        maxheappush(self.heap, (timestamp, price))
        maxheappush(self.max_heap, (price, timestamp))
        heappush(self.min_heap, (price, timestamp))
        self.keys.add(timestamp)
        if is_update:
            self.updated[timestamp] = price

    def current(self) -> int:
        while True:
            ts, val = maxheappeek(self.heap)
            if ts in self.updated:
                if self.updated[ts] == val:
                    return val
                else:
                    maxheappop(self.heap)
            else:
                return val

    def maximum(self) -> int:
        while True:
            val, ts = maxheappeek(self.max_heap)
            if ts in self.updated:
                if self.updated[ts] == val:
                    return val
                else:
                    maxheappop(self.max_heap)
            else:
                return val

    def minimum(self) -> int:
        while True:
            val, ts = self.min_heap[0]
            if ts in self.updated:
                if self.updated[ts] == val:
                    return val
                else:
