...
class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while left <= right:
            ...
            if right - left == 1:
                return min(
                    _min, nums[left] if nums[left] < nums[right] else nums[right]
                )
            if left_is_sorted:
...
assert sol.findMin([-5, -4, -3, -2, -1]) == -5
assert sol.findMin([-2, -1, -5, -4, -3]) == -5
assert sol.findMin([3, 4, 5, 6, 7, 8, 9, 10, 1, 2]) == 1
assert sol.findMin([10, 1, 2, 3, 4, 5, 6, 7, 8, 9]) == 1
assert sol.findMin([-1, 0, 1, 2, 3]) == -1
assert sol.findMin([2, 3, -1, 0, 1]) == -1
