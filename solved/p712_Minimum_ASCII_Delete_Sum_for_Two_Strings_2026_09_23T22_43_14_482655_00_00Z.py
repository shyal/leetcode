# combo chain: two-sequence-align, problem 4 of 14. Run `make combos two-sequence-align` to see
# the chain and what is done this lap; the chain is saved in graph/chains/two-sequence-align.json.
"""
...
LEETCODE: Accepted (687 ms, 225.6 MB)
"""


class Solution:
    def minimumDeleteSum(self, s1: str, s2: str) -> int:
        ascii_val = lambda x: sum(ord(y) for y in x)

        @cache
        def helper_indices(i, j):
            if i < 0:
                return ascii_val(s2[: j + 1])
            if j < 0:
                return ascii_val(s1[: i + 1])
            if s1[i] == s2[j]:
                return helper_indices(i - 1, j - 1)
            return min(
                helper_indices(i - 1, j) + ord(s1[i]),
                helper_indices(i, j - 1) + ord(s2[j]),
            )

        @cache
        def helper(a, b):
            if len(a) == 0 or len(b) == 0:
                return ascii_val(a or b)
            if a[-1] == b[-1]:
                return helper(a[:-1], b[:-1])
            return min(helper(a[:-1], b) + ord(a[-1]), helper(a, b[:-1]) + ord(b[-1]))

        # strings = helper(s1, s2)
        return helper_indices(len(s1) - 1, len(s2) - 1)
