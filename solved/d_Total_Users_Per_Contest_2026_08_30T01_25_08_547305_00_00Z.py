...
Asked for help as first i forgot to wrap the subquery in (), and secondly i forgot the group by.

"count(...) is an aggregate. Without a group by, it collapses the whole of Register into one row."

So TIL that aggregates collapse the whole table into one row without a group by.
...
class Solution(SQLDrill):
    ...
    def query(self) -> str:
        return """
            select
                contest_id,
                count((select user_id from Users)) as total_users
            from Register
            group by contest_id;
        """
