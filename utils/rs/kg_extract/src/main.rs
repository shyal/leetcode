...
- A file may hold a comment block headed `# mu <version>` (older files: `# mu source (current.mu)`): the candidate wrote that solution in mu, a small language transpiled to Python, and the class under the block is the transpiler's output, which is what ran. Judge the moves from the mu source and use its names. In mu, `for i, x in xs` enumerates; `sum|max|min|count [from s] for x in xs [if c]: e` is a fold; `first x in lo..hi if ok(x)` is a binary search for the least x with ok(x); `memo f(a) =` with `| guard -> value` lines is cached recursion; `stack <- x` appends; `stack .` pops; the last line of a def is its value. Helpers the transpiler pasted above the class (cells, nbrs, table, like, pairs, levels, adjacency, indegrees, components, graph, dijkstra, Grid, deep) are the harness's, never the candidate's moves.
