...
from history.history_builder import get_solved_problems, parse_content
...
def test_all_files_present():
    ...
    for p in solved:
        ...
        if any(
            isinstance(node, ast.Assert) for node in ast.parse(parsed_content[1]).body
        ):
            print(parsed_content)
            raise AssertionError(f"Parsing issue in {p.file}")
