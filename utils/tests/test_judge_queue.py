...
kg_extract = SourceFileLoader(
    "kg_extract_q", os.path.join(KG, "kg_extract")
).load_module()

NODES: dict = {"sliding-window": {}, "prefix-sum": {}, "two-pointers": {}}
...
def test_placeholder_is_the_rep_on_the_trains_nodes(graph):
    path = write(
        graph, "d_Slide_Never_Shrink_2026_09_06T04_31_57_473199_00_00Z.py", DRILL
    )
    e = kg_extract.stub_entry(
        path, NODES, PROBLEMS, now=datetime(2026, 9, 6, 4, 32, tzinfo=timezone.utc)
    )
...
def test_placeholder_of_a_problem_is_its_canonical_walk(graph):
    path = write(
        graph,
        "p1539_Kth_Missing_2026_09_05T21_08_16_000000_00_00Z.py",
        '"""\n1539. Kth Missing\n"""\nclass Solution:\n    pass\n',
    )
    e = kg_extract.stub_entry(path, NODES, PROBLEMS)
...
def test_placeholder_of_a_failed_file_marks_no_move(graph):
    ...
    path = write(
        graph, "d_Slide_Never_Shrink_FAILED_2026_09_06T04_31_57_473199_00_00Z.py", DRILL
    )
    e = kg_extract.stub_entry(path, NODES, PROBLEMS)
...
def test_unknown_trains_are_dropped_and_the_rep_still_exists(graph):
    path = write(
        graph,
        "d_X_2026_09_06T04_31_57_473199_00_00Z.py",
        '"""\nDRILL: X\nTRAINS: no-such-node\n"""\n',
    )
    e = kg_extract.stub_entry(path, NODES, PROBLEMS)
...
def test_a_landing_judge_does_not_clobber_the_next_placeholder(graph):
    ...
    kg_lib.store_evidence_entry(
        "solved/next.py", {"date": "2026-09-06", "moves": {}, "pending": "x"}
    )
    assert "solved/next.py" not in stale
    fresh = kg_lib.store_evidence_entry(
        "solved/judged.py", {"date": "2026-09-06", "moves": {"prefix-sum": "clean"}}
    )
...
def judged_entry():
    return {
        "date": "2026-09-06",
        "problem": "drill",
        "moves": {"sliding-window": "struggled"},
...
def tracked_status(root):
    return [
        l
        for l in git(root, "status", "--porcelain").splitlines()
        if not l.startswith("??")
    ]
...
def test_the_judge_folds_from_the_next_branch_and_rebases_it(repo):
    ...
    open(os.path.join(root, "current.py"), "w").write(
    ...
    assert (
        git(root, "merge-base", "master", "2").strip()
        == git(root, "rev-parse", "master").strip()
    )
...
def test_the_judge_rebases_a_parked_branch_too(repo):
    ...
    out = kg_extract.commit_judgement([(path, judged_entry())], refit=False)
    assert out.startswith("judge: folded into")
    assert commits(root, "parked") == [
        "sleeping: 2. Parked",
        "Parked",
        "drill: Slide, Never Shrink",
    ]
    assert (
        git(root, "merge-base", "master", "parked").strip()
        == git(root, "rev-parse", "master").strip()
    )
...
def test_the_judge_never_rewrites_a_pushed_solve(repo):
    ...
    assert out.startswith(
        "judge: committed judge: Slide Never Shrink (solve already pushed"
    )
...
def test_a_branch_that_does_not_replay_keeps_the_judges_commit_on_top(repo):
    ...
    open(ev, "w").write(
        json.dumps(
            {
                "evidence": {
                    path: {"date": "2026-09-06", "moves": {"prefix-sum": "clean"}}
                }
            }
        )
    )
    ...
    assert (
        git(root, "rev-parse", "master", "2") != before
    )  # master carries the judge: commit
    assert commits(root, "master") == [
        "judge: Slide Never Shrink",
        "drill: Slide, Never Shrink",
    ]
    assert commits(root, "2") == ["hand edit", "drill: Slide, Never Shrink"]
    assert (
        not os.path.exists(os.path.join(root, ".git", "worktrees"))
        or git(root, "worktree", "list").count("\n") == 1
    )


def test_judge_titles():
    assert (
        kg_extract.judge_title(
            "solved/d_Slide_Never_Shrink_2026_09_06T04_31_57_473199_00_00Z.py"
        )
        == "Slide Never Shrink"
    )
    assert (
        kg_extract.judge_title(
            "solved/p542_01_Matrix_FAILED_2026_09_03T03_48_58_768516_00_00Z.py"
        )
        == "542. 01 Matrix"
    )
...
def test_trains_come_from_drills_json_not_the_file(graph, tmp_path):
    ...
    (tmp_path / "graph" / "drills.json").write_text(
        json.dumps(
            {
                "drills": {
                    "d9": {
                        "title": "Slide, Never Shrink",
                        "after": [],
                        "trains": ["two-pointers"],
                    }
                }
