...
def test_attempt_label_reads_the_file_then_the_moves_then_the_assist():
    ...
    assert (
        attempt_label("solved/p1.py", {"moves": {"a": "clean"}, "assist": "hint"})
        == "hint"
    )
...
def test_an_unmapped_solve_grades_nothing():
    ...
    assert problem_due("1", e)[0] == date.today() - timedelta(days=30) + timedelta(
        days=PROBLEM_GRADUATING_DAYS
    )
...
def test_a_hinted_clean_rep_pushes_the_card_out_instead_of_closing_it():
    e = ev(
        rec(1, 10, {"a": "clean"}, assist="walkthrough"),
        rec(1, 4, {"a": "clean"}, assist="hint"),
    )
    due, interval = problem_due("1", e)
    assert interval == max(
        PROBLEM_GRADUATING_DAYS + 1, round(PROBLEM_GRADUATING_DAYS * ANKI_HARD_FACTOR)
    )
...
def test_a_later_bad_attempt_opens_a_new_card():
    e = ev(
        rec(1, 30, {"a": "clean"}, assist="learning"),
        rec(1, 20, {"a": "clean"}),
