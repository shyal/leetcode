...
def spot(pnum, moves, days_ago=0, target=None):
    return {
        f"recognition/s{pnum}_{days_ago}.md": {
            "date": iso(days_ago),
            "problem": str(pnum),
            "kind": "spot",
            "target": target or next(iter(moves)),
            "moves": dict(moves),
        }
    }
...
def test_score_the_first_move_is_not_the_target():
    ...
    problems = {
        "884": problem(["string-build-transform", "counter-build", "counts-as-data"])
    }
    moves, _ = rc.score(
        ["counter-build", "counts-as-data"], "884", problems, target="counter-build"
    )
...
def test_spotted_before_sees_only_earlier_spot_reps_with_their_verdict():
    ...
    assert rc.spotted_before("7", iso(0), recog) == (
...
def test_the_node_more_problems_need_is_served():
    ...
    assert rc.due_spot(ns, problems, {}, {}, statuses, predicted={}) == (
        "ms",
        "1",
        "untested, 2 problem(s) need only it",
    )
...
def test_failed_to_recognize_comes_first_whatever_its_score():
    ...
    assert rc.due_spot(ns, problems, {}, recog, statuses, predicted={}) == (
        "bsb",
        "4",
        "failed to recognize last time, 1 problem(s) need only it",
    )
...
def test_mapped_carriers_outrank_drafted_ones():
    ...
    assert rc.spot_carriers(
        "ms", problems, {}, {}, ns, statuses, predicted=predicted
    ) == ["1", "2", "9"]
...
def test_assisted_struggled_or_drill_solves_are_not_hits():
    ...
    assert not rc.solve_hits(
        {
            "solved/p1_0.py": {
                "date": iso(0),
                "problem": "1",
                "moves": {"bsoa": "struggled"},
            }
        },
        problems,
    )
...
def test_only_the_first_solve_of_a_problem_counts():
    problems = {"1": problem(["bsoa"])}
    ev = merged(
        clean_solve(1, ["bsoa"], days_ago=40, assist="learning"),
        clean_solve(1, ["bsoa"], days_ago=2),
    )
    assert not rc.solve_hits(
        ev, problems
    )  # the first was a copy; the re-solve remembers it
...
def test_ratio_one_rep_per_every_solves():
    ...
    assert not rc.spot_due_by_ratio(
        merged(recog, spot(5, {"ms": rc.HIT})), three, every=3
    )
...
def test_force_skips_the_ratio():
    ...
    assert (
        rc.due_spot(ns, problems, {}, recog, statuses, predicted={}, force=True)[0]
        == "ms"
    )
...
def test_drafted_problems_carry_when_the_map_has_none():
    ns, problems, statuses = graph()
    predicted = {
        "9": {"title": "draft", "walks": [{"moves": ["bsoa"]}]},
        "10": {"title": "two walks", "walks": [{"moves": ["bsoa"]}, {"moves": ["ms"]}]},
        "11": {"title": "gap", "walks": [{"moves": ["bsoa"], "missing": ["x"]}]},
    }
    evidence = merged(
        solve(1, days_ago=20), solve(2, days_ago=20), solve(3, days_ago=20)
    )
    recog = miss(1760, "bsoa", days_ago=1)
    assert rc.due_spot(
        ns, problems, evidence, recog, statuses, predicted=predicted
...
def test_html_to_markdown_keeps_emphasis_code_lists_images():
    html = (
        "<p>Given <strong>an array </strong>and <em> a value</em> <code>k</code>, "
...
def test_reveal_names_the_problem_and_the_verdict():
    rec = {
        ...
        "walk": ["monotonic-stack"],
        "moves": {"monotonic-stack": rc.MISSED},
        "seconds": 140,
        "named": ["prefix-sums"],
        "false": ["prefix-sums"],
    }
...
def test_alternative_is_a_marked_hit_filed_under_spotted_walks():
    ...
    rec = {
        "problem": "594",
        "moves": {"counter-build": rc.MISSED},
        "false": ["sliding-window-variable", "sort-then-adjacent"],
    }
    named = ["sort-then-adjacent", "sliding-window-variable"]
    rc.apply_alternative(
        rec, named, problems, "matches the sort + sliding window editorial solution"
    )
    assert rec["moves"] == {
        "sort-then-adjacent": rc.HIT,
        "sliding-window-variable": rc.HIT,
    }
    assert rec["false"] == []
    ...
    assert (
        rc.recognition_status(
            "counter-build",
            {"k": {"date": iso(0), "problem": "594", "kind": "spot", **rec}},
        )[0]
        == rc.UNTESTED
    )
    out = rc.reveal({**rec, "title": "t", "walk": ["counter-build"], "seconds": 3})
    assert (
        "hit through an alternative walk, not yet evidenced by code: sort-then-adjacent, sliding-window-variable"
        in out
    )
...
def test_alternative_mixed_with_walk_moves_keeps_both_as_hits():
    ...
    rec = {"problem": "120"}
    rec["moves"], rec["false"] = rc.score(
        named, "120", problems, target="dp-1d-rolling"
    )
    assert rec["moves"] == {"dp-state-formulate": rc.HIT, "dp-1d-rolling": rc.MISSED}
    assert rec["false"] == ["memoize-recursion"]
    rc.apply_alternative(
        rec, named, problems, "top-down with @cache is the accepted memoized solution"
    )
...
def test_an_alternative_marks_the_target_and_two_in_a_row_leave_it_to_solves():
    ...
    rec = {
        "problem": "120",
        "moves": {"dp-1d-rolling": rc.MISSED},
        "false": ["memoize-recursion"],
    }
    rc.apply_alternative(
        rec, ["memoize-recursion"], problems, "why", target="dp-1d-rolling"
    )
    assert rec["moves"] == {
        "memoize-recursion": rc.HIT,
        "dp-1d-rolling": rc.ALTERNATIVE,
    }
    one = merged(
        miss(1235, "dp-1d-rolling", days_ago=5),
        spot(
            120, {"dp-1d-rolling": rc.ALTERNATIVE}, days_ago=2, target="dp-1d-rolling"
        ),
    )
    # status still failed (from the park), date moved, still served
    assert rc.recognition_status("dp-1d-rolling", one) == (
        rc.FAILED_TO_RECOGNIZE,
    ...
    two = merged(
        one,
        spot(
            1646, {"dp-1d-rolling": rc.ALTERNATIVE}, days_ago=1, target="dp-1d-rolling"
        ),
    )
    ...
    statuses = {n: (SOLID, date.today()) for n in ns}
    pick = rc.due_spot(
        ns, {"7": problem(["dp-1d-rolling"])}, {}, two, statuses, predicted={}
    )
...
def test_difficulty_restricts_the_carrier_tier():
    ns, problems, statuses = graph()
    assert rc.spot_carriers(
        "ms", problems, {}, {}, ns, statuses, predicted={}, difficulty="Hard"
    ) == ["2"]
    assert rc.due_spot(ns, problems, {}, {}, statuses, predicted={}, difficulty="Hard")[
        :2
    ] == ("ms", "2")
    assert rc.due_spot(
        ns, problems, {}, {}, statuses, predicted={}, difficulty="Medium"
    )[:2] == ("bsoa", "3")
...
def test_a_failed_route_reveals_nothing():
    rec = {
        ...
        "target": "sort-by-custom-key",
        "walk": ["sort-by-custom-key", "binary-search-boundary", "dp-1d-rolling"],
        "moves": {"sort-by-custom-key": rc.HIT},
        "named": ["sort-by-custom-key"],
        "false": ["heap-simulation"],
        "seconds": 536,
        "valid": False,
...
def test_a_hit_on_the_same_day_as_a_miss_wins():
    recog = merged(
        miss(1760, "spc", days_ago=0),
        {
            "solved/p2177_0.py#solve": {
                "date": iso(0),
                "problem": "2177",
                "kind": "solve",
                "moves": {"spc": rc.HIT},
