...
REFERENCES = sorted(glob.glob(os.path.join(ROOT, "graph", "node_notes", "*", "d[0-9]*.py")))
...
def test_reference_passes_its_drill(reference, tmp_path):
    ...
    env = dict(os.environ, PYTHONPATH=os.pathsep.join([ROOT, os.path.join(ROOT, "utils"), os.path.join(ROOT, "utils", "harness")]))
    run = subprocess.run([PY, str(script)], capture_output=True, text=True, env=env, cwd=ROOT, timeout=120)
